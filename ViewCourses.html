
<!DOCTYPE html>
<html>

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title></title>
    <link rel="stylesheet" href="MyStyle.css">
</head>

<body>
    <header>
        <div class="container-1">
            <nav>
                <ul>
                    <li> <a href="Home.html">Personal Information</a></li>
                    <li> <a class="active" href="ViewCourses.html">Courses Information</a></li>
                    <li><a href="ViewExperience.html"> Experiences Information</a> </li>
                    <li> <a href="AddCourse.html">Add Course</a></li>
                    <li> <a href="AddExperience.html">Add Experience</a></li>
                </ul>
            </nav>
            <div class="logo"><img src="Images/Azhar_WHITE_LOGO.png" alt="Azhar University"> </div>
        </div>
    </header>
    <!-- End header -->
    <div class="table-1">
        <div class="container-2">All Course Information</div>
    </div>
    <section id="table">
        <table >
            <thead>
                <tr>
                    <th class="th" rowspan="2">#</th>
                    <th class="th" rowspan="2">Course Name</th>
                    <th class="th" rowspan="2">Total Hours</th>
                    <th class="th" colspan="2">Date</th>
                    <th class="th" rowspan="2">Institution</th>
                    <th class="th" rowspan="2">Attachment</th>
                    <th class="th" rowspan="2">Notes</th>
                <tr>
                    <th>From</th>
                    <th>to</th>
                </tr>
                </tr>
            </thead>
            <tbody>
                <tr bgcolor="#E5E5E5">
                    <td>1</td>
                    <td>JAVA</td>
                    <td>50</td>
                    <td>1/8/2021</td>
                    <td>25/10/2021</td>
                    <td>Vision Plus</td>
                    <td><a href="Course_1View.html">view</a></td>
                    <td>Establishing</td>
                </tr>
                <tr>
                    <td>2</td>
                    <td>Electronics Maintinence</td>
                    <td>30</td>
                    <td>1/2/2020</td>
                    <td>4/4/2020</td>
                    <td>Omantel</td>
                    <td><a href="Course_2View .html">view</a></td>
                    <td>Protection</td>
                </tr>
                <tr bgcolor="#E5E5E5">
                    <td>3</td>
                    <td>Depat</td>
                    <td>100</td>
                    <td>1/11/2021</td>
                    <td>30/3/2021</td>
                    <td>Vision Plus</td>
                    <td><a href="Course_3View  .html">view</a> </td>
                    <td>Development</td>
                </tr>
                <tr>
                    <td>4</td>
                    <td>LapMaintinence</td>
                    <td>20</td>
                    <td>1/10/2021</td>
                    <td>30/12/2021</td>
                    <td>University</td>
                    <td><a href="Course_4View .html">view</a> </td>
                    <td>The design</td>
                </tr>
                <tr bgcolor="#E5E5E5">
                    <td>5</td>
                    <td>C#</td>
                    <td>50</td>
                    <td>1/1/2021</td>
                    <td>30/30/2021</td>
                    <td>Vision Plus</td>
                    <td>view</td>
                    <td>Programming</td>
                </tr>
                <tr>
                    <td>6</td>
                    <td>Data Analysis</td>
                    <td>50</td>
                    <td>25/10/2019</td>
                    <td>2/1/2020</td>
                    <td>cyber school</td>
                    <td>view</td>
                    <td>Planning</td>
                </tr>
                <tr bgcolor="#E5E5E5">
                    <td>7</td>
                    <td>PMP</td>
                    <td>50</td>
                    <td>1/4/2021</td>
                    <td>30/6/2021</td>
                    <td>Shehab Abo El Azm</td>
                    <td>view</td>
                    <td>Management</td>
                </tr>
                <tr>
                    <td>8</td>
                    <td>ِAccess</td>
                    <td>100</td>
                    <td>15/10/2018</td>
                    <td>1/3/2019</td>
                    <td>Amideast</td>
                    <td>view</td>
                    <td>The language</td>
                </tr>
            </tbody>
        </table>
    </section>
</body>

</html>